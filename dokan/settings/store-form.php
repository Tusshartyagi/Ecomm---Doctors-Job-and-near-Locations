...
$gphclogo_id    = ! empty( $profile_info['gphclogo'] ) ? $profile_info['gphclogo'] : 0;
...
$setting_show_public     = isset( $profile_info['setting_show_public'] ) ? $profile_info['setting_show_public'] : 'no';
...
$wholesellerAuthor = get_user_meta( $loggedInUser, 'dokan_wholeseller_exists', true); 
?>
...
<form method="post" id="store-form" action="" class="dokan-form-horizontal">
    ...
    <div class="dokan-form-group">
	 ...
	 <div class="dokan-form-group">
        <label class="dokan-w3 dokan-control-label" for="dokan_gphclogo"><?php esc_html_e( 'GPHC Logo', 'dokan-lite' ); ?></label>

        <div class="dokan-w5 dokan-gravatar">
            <div class="dokan-left custom_gphclogo gravatar-wrap<?php echo $gphclogo_id ? '' : ' dokan-hide'; ?>">
                <?php $gphclogo_url = $gphclogo_id ? wp_get_attachment_url( $gphclogo_id ) : ''; ?>
                <input type="hidden" class="dokan-file-field doken_gphc_val" value="<?php echo esc_attr( $gphclogo_id ); ?>" name="dokan_gphclogo">
                <img class="dokan-gravatar-img dokan-gravatar-img-gphc" src="<?php echo esc_url( $gphclogo_url ); ?>">
                <a class="dokan-close dokan-remove-gravatar-image">&times;</a>
            </div>
            <div class="button_hide_gphc gravatar-button-area<?php echo esc_attr( $gphclogo_id ) ? ' dokan-hide' : ''; ?>">
                <a href="#" class="dokan-pro-gphclogo-drag dokan-btn dokan-btn-default"><i class="fas fa-cloud-upload-alt"></i> <?php esc_html_e( 'Upload Photo', 'dokan-lite' ); ?></a>
            </div>
        </div>
    </div>
	
		<?php  if($wholesellerAuthor == 'on') { ?>
        <div class="dokan-form-group">
            <label class="dokan-w3 dokan-control-label"><?php esc_html_e( 'Sell direct to public', 'dokan-lite' ); ?></label>
            <div class="dokan-w5 dokan-text-left">
                <div class="checkbox">
                    <label>
                        <input type="hidden" name="setting_show_public" value="no">
                        <input type="checkbox" name="setting_show_public" value="yes"<?php checked( $setting_show_public, 'yes' ); ?>> <?php esc_html_e( 'Sell direct to public', 'dokan-lite' ); ?>
                    </label>
                </div>
            </div>
        </div>
            ...
            <div class="dokan-w5 dokan-text-left dokan_tock_check">
                <div class="checkbox">
                    <label for="dokan-store-time-enable" class="control-label">
                        ...
                        <?php esc_html_e( 'Store has opening and closing times', 'dokan-lite' ); ?>
                    </label>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    (function($) {
		  ...
		  $('.dokan-pro-gphclogo-drag').on( 'click', function ( e ) { e.preventDefault();
        var upload = wp.media({
        ...
        .on('select', function(){
            ...
            $('img.dokan-gravatar-img-gphc').attr('src',attach.url);
            $('.doken_gphc_val').val(attach.id);
            $('.custom_gphclogo').removeClass('dokan-hide');
            $('.button_hide_gphc').addClass('dokan-hide');
        })
        .open();
   });
